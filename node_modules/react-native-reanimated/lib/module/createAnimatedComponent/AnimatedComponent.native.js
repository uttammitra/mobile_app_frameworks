...
let id = 0;
const FORCE_REACT_RENDER_FOR_SETTLED_ANIMATIONS = getStaticFeatureFlag('FORCE_REACT_RENDER_FOR_SETTLED_ANIMATIONS') && !getStaticFeatureFlag('USE_ANIMATION_BACKEND');
export default class AnimatedComponent extends ReanimatedAnimatedComponent {
  ...
  _syncStylePropsBackToReact(settledUpdates) {
    if (!FORCE_REACT_RENDER_FOR_SETTLED_ANIMATIONS) {
      return;
    }
    ...
    // style-only.
    const animatedPropsKeys = new Set([...this._animatedProps.flatMap(animatedProp => Object.keys(animatedProp.initial?.value ?? {})), ...Object.keys(this._InlinePropManager._inlineTopLevelProps)]);
...
//# sourceMappingURL=AnimatedComponent.native.js.map